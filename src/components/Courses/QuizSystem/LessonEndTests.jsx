...
export const LessonEndTestsSection = ({ lesson, lessonStatuses, id, course, isLive = false }) => {
  ...
  const testType = isLive ? "Lesson-End Test (Live Session)" : "Lesson-End Test (Video Session)";
  const lessonEndTests = lesson.lesson_end_tests.filter(
    (test) => test.test_type === testType
  );
  ...
  const basePath = isLive ? `/live-courses/${id}` : `/courses/${id}`;
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        {lessonEndTests.map((test, idx) => (
          <div
            ...
            onClick={() => {
              if (canTakeTest) {
                navigate(`${basePath}/test/lesson/${test.id}`, {
                  state: {
                    course,
                    test,
                    lessonId: lesson.id,
